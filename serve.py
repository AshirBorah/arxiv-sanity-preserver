...
def get_username(user_id):
    ...
    rv = query_db("select username from user where user_id = ?", [user_id], one=True)
    return rv[0] if rv else None
    ...
    if g.user:
        ...
        if recent_days is not None:
            ...
            out = [
                x
                for x in out
                if curtime - x["time_updated"] < recent_days * 24 * 60 * 60
            ]
    return out
...
def discuss():
    ...
    for c in comms:
        cc = [
            tags_collection.count({"comment_id": c["_id"], "tag_name": t}) for t in TAGS
        ]
...
def discussions():
    ...
    comms_cursor = (
        comments.find().sort([("time_posted", pymongo.DESCENDING)]).limit(100)
    )
...
def login():
    username = request.get_json().get("body").get("username")
    password = request.get_json().get("body").get("password")
...
def signup():
    ...
    if get_user_id(req.get("username")) is not None:
        ...
        flash("User already exists: " + req.get("username") + "\n Is this you?")
    else:
        ...
        g.db.execute(
            ...
            [
                ...
                req.get("institution"),
                req.get("username"),
                generate_password_hash(req.get("password")),
                creation_time,
            ],
        )
