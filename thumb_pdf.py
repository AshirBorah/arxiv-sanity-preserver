...
if not shutil.which("convert"):  # shutil.which needs Python 3.3+
    print(
        "ERROR: you don't have imagemagick installed. Install it first before calling this script"
    )
    sys.exit()
...
pdf_files = [
    x for x in files_in_pdf_dir if x.endswith(".pdf")
]  # filter to just pdfs, just in case
print("PDF: ", pdf_files)
...
for i, p in enumerate(pdf_files):
    ...
    if not os.path.isfile(os.path.join(Config.tmp_dir, "thumb-0.png")):
        ...
        cmd = "mv %s %s" % (os.path.join(Config.tmp_dir, "thumb-*.png"), thumb_path,)
        print(cmd)
        os.system(cmd)
